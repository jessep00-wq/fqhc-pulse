**Subject A:** the 14-page binder reviewers actually open
**Subject B:** sample HRSA evidence packet (no signup)
**Preview:** Sharing the binder template my last CHC used to pass OSV with zero conditions.

---

Hi {{first_name}},

Following up on my note last week — no reply needed, just wanted to leave you with something useful either way.

This is the actual binder export my last CHC handed reviewers during their OSV. Zero conditions, zero progressive action. 14 pages, every UDS measure tied to a PDSA cycle and a board minute. No login, no email gate:

→ https://measurewise.org/MeasureWise_Sample_Export.pdf?utm_source=cold-email&utm_campaign=osv-q1-2026

Two things reviewers told me they look for and most binders miss:
1. **PDSA close-out language tied to the next cycle** — "we tried X, measured Y, here's what we changed for the next quarter." Most binders stop at "we tried X."
2. **Board minutes that quote the measure** — not "QI report presented," but "QI report on CMS124 cervical cancer screening presented; goal 70%, current 58%; intervention approved."

Steal the format. If your team wants help wiring up the workflow that produces this binder automatically from your daily QI work, that's what MeasureWise does — happy to show you in 15 minutes.

— Jessica

Jessica R. Smith, BSN
Founder, MeasureWise
measurewise.org · jessica@measurewise.org
