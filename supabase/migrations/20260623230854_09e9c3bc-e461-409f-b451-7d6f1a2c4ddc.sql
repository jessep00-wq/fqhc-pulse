...
WITH CHECK (
  ...
  AND (
    ...
    OR NOT EXISTS (SELECT 1 FROM public.organizations o WHERE o.owner_id = auth.uid())
  )
);
...
ALTER TABLE public.playbook_leads
  DROP CONSTRAINT IF EXISTS playbook_leads_email_format_chk,
  ADD CONSTRAINT playbook_leads_email_format_chk
  CHECK (work_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$' AND length(work_email) <= 254),
  DROP CONSTRAINT IF EXISTS playbook_leads_full_name_len_chk,
  ADD CONSTRAINT playbook_leads_full_name_len_chk
